...
var MappingsForceUpdate bool      // flag will trigger a cache update when set
var cachedMappings *Mappings      // global cachedMappings
var cachedMappingsErr error       // any error that occurred when retrieving that last global cachedMappings
var cachedMappingsMu sync.RWMutex // for synchronizing cache updates
var getMappingsFunc = GetMappings // allows the GetMappings to be swapped out for a mocked function

// GetCachedMappings() facilitates a global cached mappings structure generated by GetMappings().
// Only use GetCachedMappings() when the operation is fault-tolerant or retry has been configured.
// The mappings will be updated when mappings is nil, MappingsForceUpdate flag is set, or when the
// cached value is older than 60 seconds. Mappings will also immediately update when an error
// occurred while getting the current mappings and, therefore, should not be called when
// ErrMappingRSTs is expected; if ErrMappingRSTs is expected then just call GetMappings().
func GetCachedMappings(ctx context.Context) (*Mappings, error) {
	cachedMappingsMu.RLock()
	if cachedMappings != nil && cachedMappingsErr == nil && !MappingsForceUpdate {
		// Hold read-only lock until background update starts
		defer cachedMappingsMu.RUnlock()
...
func updateCachedMappingsInBackground(ctx context.Context) {
	...
	activeCachedMappingsUpdateMu.Lock()
	if activeCachedMappingsUpdate {
		activeCachedMappingsUpdateMu.Unlock()
		return
	}

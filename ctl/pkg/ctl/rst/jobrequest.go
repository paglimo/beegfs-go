...
func SubmitJobRequest(ctx context.Context, cfg JobRequestCfg, chanSize int) (<-chan *JobResponse, context.CancelFunc, error) {
	...
	go func() {
		...
		sendJobRequest(ctx, beegfs, nil, &cfg, requestChan)
	}()
...
func sendJobRequest(ctx context.Context, beegfs filesystem.Provider, mappings *util.Mappings, cfg *JobRequestCfg, requestChan chan<- *jobRequestWithError) {
	...
	// Check for an existing file at the destination path. Update pathInfo if the file exists.
	if cfg.Download && pathInfo.IsDir {
		destPath := mapRemoteToLocalPath(beegfs.GetMountPath(), cfg.RemotePath, cfg.Flatten)
		if destPathInfo, err := getMountPathInfo(beegfs, destPath); err != nil {
			if !errors.Is(err, fs.ErrNotExist) {
				sendError(fmt.Errorf("failed to determine if path exists: %w", err))
			}
		} else if destPathInfo.Exists && !destPathInfo.IsDir {
			pathInfo = destPathInfo
		}
	}

	jobType, err := getJobType(beegfs, &pathInfo, cfg)
	if err != nil {
		sendError(fmt.Errorf("unable to determine job type! This is probably a bug: %w", err))
		return
	}
	if jobType == JobTypeUnspecified {
		sendError(fmt.Errorf("unspecified job type! This is probably a bug"))
		return
	}

	switch jobType {
	case JobTypeDirSync:
		sendDirSyncJobRequest(pathInfo.Path, cfg, requestChan)
	case JobTypeSync:
		...
		if validRstId(cfg.RSTID) {
			sendSyncJobRequest(pathInfo.Path, cfg.RSTID, cfg, requestChan)
		} else {
			rstIds, err := getEntryRSTs(entry.Entry)
			if err != nil {
				sendError(err)
			}
			for _, rstId := range rstIds {
				sendSyncJobRequest(pathInfo.Path, rstId, cfg, requestChan)
			}
		}
	}
}

// getJobType determine which job request to send. If cfg.JobType is specified then it is
// immediately returned; otherwise, the job type is determined based on the local beegfs
// path information and job request configuration.
func getJobType(beegfs filesystem.Provider, pathInfo *mountPathInfo, cfg *JobRequestCfg) (JobType, error) {
	if cfg.JobType != JobTypeUnspecified {
		return cfg.JobType, nil
	}

	if cfg.Download {
		if pathInfo.IsDir {
			return JobTypeDirSync, nil
		} else if pathInfo.Exists {
			return JobTypeSync, nil
		}
		if parentPathInfo, err := getMountPathInfo(beegfs, filepath.Dir(cfg.Path)); err != nil {
			return JobTypeUnspecified, fmt.Errorf("failed to determine the parent path info: %w", err)
		} else if parentPathInfo.IsDir {
			return JobTypeSync, nil
		}
	...
	return JobTypeUnspecified, fmt.Errorf("failed to determine job type: %w", fs.ErrNotExist)
}

func sendSyncJobRequest(inMountPath string, rstId uint32, cfg *JobRequestCfg, requestChan chan<- *jobRequestWithError) {
	...
	request := &beeremote.JobRequest{Path: inMountPath, RemoteStorageTarget: rstId}
	request.Type = &beeremote.JobRequest_Sync{
		Sync: &flex.SyncJob{
	...
	requestChan <- &jobRequestWithError{Request: request}
}

func sendDirSyncJobRequest(inMountPath string, cfg *JobRequestCfg, requestChan chan<- *jobRequestWithError) {
	var operation flex.DirSyncJob_Operation
	if cfg.Download {
		operation = flex.DirSyncJob_DOWNLOAD
	} else {
		operation = flex.DirSyncJob_UPLOAD
	}

	request := &beeremote.JobRequest{
		Path:                inMountPath,
		RemoteStorageTarget: cfg.RSTID,
		Type: &beeremote.JobRequest_DirSync{
			DirSync: &flex.DirSyncJob{
				RemotePath: cfg.RemotePath,
				Operation:  operation,
				Overwrite:  cfg.Overwrite,
				Flatten:    cfg.Flatten,

...
// InputMethod is used to configure how paths are provided to processEntries(). It must be
// initialized using DetermineProcessingMethod() before first use.
...
// DetermineInputMethod() processes user configuration to determine how paths are provided to
...
func DetermineInputMethod(paths []string, recurse bool, stdinDelimiter string) (InputMethod, error) {
	pm := InputMethod{}
	if pathsLen := len(paths); pathsLen == 0 {
		...
		if paths[0] == "-" {
			...
			pm.stdinDelimiter, err = util.GetStdinDelimiterFromString(stdinDelimiter)
			if err != nil {
				return pm, err
			}
		} else if recurse {
...
// processEntries() processes one or more entries based on the InputMethod and executes the
// specified request for each entry by invoking the provided processEntry() function.
//
// It handles any setup needed to read from the specified InputMethod (such as reading from stdin)
// and by default processes entries in parallel based on the global num-workers flag. Because
...
// the Goroutine responsible for providing the paths based on the InputMethod (for example errors
// reading from stdin), and the Goroutines executing the processEntry function. Thus callers should
// not wait for this channel to be closed indefinitely, and instead rely on the ResultT channel to
// determine when all entries have been processed.
func processEntries[ResultT any](
	ctx context.Context,
	method InputMethod,
	singleWorker bool,
